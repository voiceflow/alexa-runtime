...
describe('payment state handler unit tests', () => {
  ...
  describe('canHandle', () => {
    it('false', () => {
      ...
      const result = paymentStateHandler.canHandle(null as any, runtime as any, null as any, null as any);
  ...
  describe('handle', () => {
    describe('success path', () => {
      it('ACCEPTED', () => {
        ...
        const runtime = { storage: { get: sinon.stub().returns(payment), delete: sinon.stub() } };

        const result = paymentStateHandler.handle(null as any, runtime as any, null as any, null as any);
        expect(result).to.eql(payment.successPath);
        expect(runtime.storage.get.args).to.eql([[S.PAYMENT]]);
        expect(runtime.storage.delete.args).to.eql([[S.PAYMENT]]);
      });
