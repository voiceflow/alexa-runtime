  ...
  describe('getCommand', () => {
    ...
    describe('request type intent', () => {
      ...
      it('with extracted frame', () => {
        ...
        expect(getCommand(runtime as any, extraFrameCommand as any)).to.eql({ ...frame, intent: payload.intent });
      });
    });
  });
    ...
    describe('has command', () => {
      ...
      describe('next', () => {
        ...
        it('not last frame', () => {
          ...
          expect(runtime.trace.debug.args).to.eql([[`matched intent **${res.command.intent}** - exiting flows and jumping to node`]]);
        });
